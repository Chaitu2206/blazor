...
namespace Zindagi
{
    public static class ZindagiSeedWorkExtensions
    {
        ...
        public static Result<VendorId> GetOpenId(this ClaimsPrincipal claimsPrincipal)
        {
        ...
        public static Result<long> GetSerialNumber(this ClaimsPrincipal claimsPrincipal)
        {
            var serialNumber = claimsPrincipal.Claims.FirstOrDefault(q => q.Type.Equals(ClaimTypes.SerialNumber, StringComparison.Ordinal))?.Value ?? string.Empty;
            return long.Parse(serialNumber, CultureInfo.InvariantCulture);
        }
